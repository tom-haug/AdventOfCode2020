...
def load_property_definitions_and_tickets_from_file(file_name: str):
    ...
    return property_definitions, your_ticket, nearby_tickets


def get_invalid_property_count_on_tickets(property_definitions: list[TicketPropertyDefinition], tickets: list[Ticket]) -> int:
    invalid_property_values_on_tickets: list[int] = []
    for cur_ticket in tickets:
        invalid_property_values_on_tickets += cur_ticket.all_invalid_property_values(property_definitions)

    return sum(invalid_property_values_on_tickets)


def filter_valid_tickets(tickets: list[Ticket], property_definitions: list[TicketPropertyDefinition]) -> list[Ticket]:
    valid_tickets: list[Ticket] = []
    for cur_ticket in tickets:
        invalid_values_on_nearby_tickets = cur_ticket.all_invalid_property_values(property_definitions)
        if not any(invalid_values_on_nearby_tickets):
            valid_tickets.append(cur_ticket)
    return valid_tickets


def filter_definition_description(property_definitions: list[TicketPropertyDefinition], search_description: str) -> list[TicketPropertyDefinition]:
    return [property_definition for property_definition in property_definitions if search_description in property_definition.description]
...
def load_property_positions(property_definitions: list[TicketPropertyDefinition], tickets: list[Ticket]):
    keep_going = True
    while keep_going:
        keep_going = deduce_next_property_position(property_definitions, tickets)